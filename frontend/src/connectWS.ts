...
function sendReaction(messageId: string, emoji: string): void {
    ...
    const updatedReactions = (messageCache[messageIndex].reactions || []).filter(
        r => r.from.username !== currentUser
    );
...
function displayMessage(sender: string, message: string, messageId: string, reactions?: ChatReaction[]): void {
    ...
    reactionBtn.onclick = (e) => {
        ...
        const close = (ev: MouseEvent) => {
            if (!reactionsDiv.contains(ev.target as Node)) {
                reactionsDiv.remove();
...
function updateUserCount(name: string): void {
