...
const userSockets: Map<string, WebSocket> = new Map(); // username -> WebSocket
...
async function publishToRabbitMQ(payload: any, isDM: boolean = false, recipient?: string) {
  if (!channel) {
...
function formatDbMessageToChatMessage(
  msg: DbMessageRow & { reactions?: DbReactionRow[] },
  reactionsRows: DbReactionRow[] = msg.reactions || []
): ChatMessage {
...
wss.on('connection', async ws => {
  ...
  ws.on('message', async data => {
    ...
    try {
      ...
      if (parsed.type === 'chat') {
        const sender: string = parsed.sender || user?.username || 'anonymous';
        const messageText: string = parsed.message;
        const recipient: string | undefined = parsed.recipient || null;
